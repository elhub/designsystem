# @elhub/ds-graph

## 6.0.1

### Patch Changes

- Test automated GitHub Actions by running changeset publish from root

## 6.0.0

### Major Changes

- replace highcharts-react-official integration with @highcharts/react to avoid dynamic react require errors
- migration guide: https://www.highcharts.com/docs/react/v4-migration-guide

## 5.1.2

### Patch Changes

- fix highcharts module imports for React 19 upgrade compatibility and add React peer dependencies

## 5.1.1

### Patch Changes

- Updated dependencies
  - @elhub/ds-tokens@7.0.0

## 5.1.0

### Minor Changes

- 6f75b8c: Dependency upgrades

### Patch Changes

- Updated dependencies [6f75b8c]
  - @elhub/ds-tokens@6.2.0

## 5.0.3

### Patch Changes

- Updated dependencies
  - @elhub/ds-tokens@6.0.0

## 5.0.2

### Patch Changes

- Add highcharts as a peer dependency

## 5.0.1

### Patch Changes

- Expose all types from highcharts

## 5.0.0

### Major Changes

- chore: bump highcharts to v12

## 4.1.1

### Patch Changes

- Updated dependencies
  - @elhub/ds-tokens@5.0.0

## 4.1.0

### Minor Changes

- Perform deep merge to keep nested base options

## 4.0.2

### Patch Changes

- Add general styling and configuration to chart component

## 4.0.1

### Patch Changes

- Build-config-fixes to allow consumers to bump to React v19

## 4.0.0

### Patch Changes

- 9aa72d7: Delete unused and unexported code. Improve code quality by proper typing
- cf3e431: Moving chart series colors to ds-tokens, re-exporting them for reuse
- Updated dependencies [ff9a56e]
- Updated dependencies [239ca18]
- Updated dependencies [9aa72d7]
- Updated dependencies [b32be98]
- Updated dependencies [9d05d89]
- Updated dependencies [cf3e431]
- Updated dependencies [bbb3477]
- Updated dependencies [d0859bd]
- Updated dependencies [c71c6b1]
- Updated dependencies [477ae8c]
  - @elhub/ds-tokens@4.0.0

## 4.0.0-next.1

### Patch Changes

- Delete unused and unexported code. Improve code quality by proper typing
- Updated dependencies
  - @elhub/ds-tokens@4.0.0-next.8

## 4.0.0-next.0

### Patch Changes

- Moving chart series colors to ds-tokens, re-exporting them for reuse
- Updated dependencies
  - @elhub/ds-tokens@4.0.0-next.4

## 3.0.0

### Major Changes

- Set version 3 for all packages in order to introduce a pre-release strategy
